...
BOOL process_name_to_pid(__out DWORD& pid, __in const std::wstring& process_name) {
	...
	if (snapshot != INVALID_HANDLE_VALUE) {
		...
		do {
			if (!_tcsicmp(process_name.c_str(), entry.szExeFile)) {	//<string.h> 
...
BOOL InjectDll(DWORD64 dwPID, LPCTSTR szDllPath)
{
	HANDLE                  hProcess, hThread;
	LPVOID                  pRemoteBuf;
	DWORD64                   dwBufSize = (DWORD64)(_tcslen(szDllPath) + 1) * sizeof(TCHAR);
...
int _tmain(int argc, TCHAR* argv[])
{
	...
	if (process_name_to_pid(pid, L"HxD.exe")) {
		// Inject(Eject) Dll to all process
		if (!_tcsicmp(L"-i", L"-i"))
			nMode = EJECTION_MODE;

		if (nMode) {
			InjectDll(pid, L"JiHoon.dll");
			printf("Injected\n");
		}
		else {
			EjectAllProcess(L"JiHoon.dll");
			printf("Ejected\n");
		}
	}
	else {
